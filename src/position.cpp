...
*/
    ...
    if (count<PAWN>(WHITE) > 5)
        return PositionSetError("Unsupported position. WHITE has more than 5 pawns.");
    if (count<PAWN>(BLACK) > 5)
        return PositionSetError("Unsupported position. BLACK has more than 5 pawns.");
    for (Color c : {WHITE, BLACK})
        if (count<ROOK>(c) > 2)
            return PositionSetError(std::string("Unsupported position. ")
                                    + (c == WHITE ? "WHITE " : "BLACK ")
                                    + "has more than 2 rooks.");
        else if (count<ADVISOR>(c) > 2)
            return PositionSetError(std::string("Unsupported position. ")
                                    + (c == WHITE ? "WHITE " : "BLACK ")
                                    + "has more than 2 advisors.");
        else if (count<CANNON>(c) > 2)
            return PositionSetError(std::string("Unsupported position. ")
                                    + (c == WHITE ? "WHITE " : "BLACK ")
                                    + "has more than 2 cannons.");
        else if (count<KNIGHT>(c) > 2)
            return PositionSetError(std::string("Unsupported position. ")
                                    + (c == WHITE ? "WHITE " : "BLACK ")
                                    + "has more than 2 knights.");
        else if (count<BISHOP>(c) > 2)
            return PositionSetError(std::string("Unsupported position. ")
                                    + (c == WHITE ? "WHITE " : "BLACK ")
                                    + "has more than 2 bishops.");
